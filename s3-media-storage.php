...
class S3MS {
    ...
    public static function s3_attachment_url($url, $post_id) 
    {
    ...
    public static function s3_delete_attachment($url) 
    { 
        $settings = json_decode(get_option('S3MS_settings'), true);
    ...
    public static function s3_image_make_intermediate_size($attachment_path) 
    {
        $upload_dir = wp_upload_dir();
    ...
    public static function s3_update_attachment_metadata($data, $attachment_id) 
    {
...
add_filter('wp_update_attachment_metadata', array('S3MS', 's3_update_attachment_metadata'), 9, 2);
// Handle thumbs that are created for that image
add_filter('image_make_intermediate_size', array('S3MS', 's3_image_make_intermediate_size'));
// Handle when image urls are requested.
add_action("wp_get_attachment_url", array('S3MS', 's3_attachment_url'), 9, 2);
add_action("wp_get_attachment_thumb_url", array('S3MS', 's3_attachment_url'), 9, 2);
// Handle when images are deleted.
add_action("wp_delete_file", array('S3MS', 's3_delete_attachment'));
